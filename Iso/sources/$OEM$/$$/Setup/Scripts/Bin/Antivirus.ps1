...
$Script:JobsInitialized = $false # Flag to indicate if background jobs have started
...
function Write-ErrorLog {
    ...
    # FAIL-SAFE MODE: Only trigger for runtime critical errors, not initialization
    # Check if we're past initialization (jobs are running)
...
function Write-Log {
    ...
    try {
        ...
        Write-Host "[WARNING] Failed to write log: $message - Error: $_"
    }
}
...
function Write-SecurityEvent {
    ...
    try {
        ...
        $securityLogPath = "$quarantineFolder\security_events.jsonl"
        $eventJson | Out-File $securityLogPath -Append -Encoding UTF8
        
        # Also write to Windows Event Log if possible
        try {
            ...
            # Silently fail if we can't write to Windows Event Log (requires admin)
        }
    } catch {
    ...
    Start-Job -ScriptBlock {
        ...
        while ($true) {
            ...
            try {
                ...
                # Check for processes trying to connect to our monitoring infrastructure
